...
export default function HomeClient({ params }: PageProps) {
          ...
          >
            ...
            >
              <div className="flex items-center gap-1.5">
                ...
                <span className="font-semibold text-foreground ml-1">{t('landing.rating')}</span>
              </div>
              <span className="text-border/40 hidden sm:inline">|</span>
              <div className="flex items-center gap-1.5">
                <ShieldCheck className="h-3.5 w-3.5 text-green-500" />
                <span>{t('landing.privateNetwork')}</span>
              </div>
              <span className="text-border/40 hidden sm:inline">|</span>
              <span>{t('landing.verificationDesk')}</span>
            </motion.div>
          </motion.div>
        ...
        <div className="mx-auto max-w-xl space-y-12">
          ...
          <div className="bg-card border border-border/20 rounded-3xl p-8 shadow-lg space-y-6">
            ...
            >
              ...
              <span>{t('landing.checkoutCta')}</span>
            </Link>
          </div>
      ...
      <section className="py-28 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto w-full">
        ...
        <div className="space-y-4 max-w-xl mx-auto">
          {faqData.map((faq, idx) => {
            ...
            const question = faq.question[localeParam as keyof typeof faq.question] || faq.question['en'] || '';
            const answer = faq.answer[localeParam as keyof typeof faq.answer] || faq.answer['en'] || '';
            return (
              <div 
                key={idx}
        ...
        <div className="text-center pt-4">
          ...
          >
            <span>{t('landing.viewAllFaqs')}</span>
